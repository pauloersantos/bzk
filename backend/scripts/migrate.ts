...
try {
  ...
  const applied = new Map<string, string>((await client.query<{ migration_id: string; checksum: string }>('select migration_id, checksum from public.schema_migrations')).rows.map((row) => [row.migration_id, row.checksum]));
  if (command === 'status') {
    ...
    for (const file of migrations) {
      ...
      if (applied.has(file)) {
        // Migrações aplicadas são imutáveis. Qualquer correção deve entrar em um
        // novo arquivo para preservar a reprodução e a auditoria do esquema.
